...
| Self-attention KV-cache decoder (24-layer) produces wrong output — real hardware only, not a compiler bug | Open — workaround identified, not yet built | W-03's decoder is silently wrong past the first decode step at full 24-layer depth; the model, ONNX graph, and RKNN's own x86 simulator (even at full depth) are all verified correct — only real RK3588 silicon diverges, and only beyond ~18 layers | Validated: splitting into two 12-layer `.rknn` models (both independently confirmed correct on real hardware) should work; not yet implemented. Closest related upstream issue: [airockchip/rknn-toolkit2#460](https://github.com/airockchip/rknn-toolkit2/issues/460) (same toolkit/target, same SDPA-fusion family, unconfirmed same root cause). | `RKNN-SA-KV-DECODER-BUG.md` |
